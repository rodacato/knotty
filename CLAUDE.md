...
- Medidas en milímetros. Arquitectura hexagonal: `src/architecture.test.ts` verifica las fronteras entre capas y entre los grupos de `src/domain/` (`design`, `materials`, `checks`, `furniture`, `editing`, `session`).
...
- Banco de pruebas: junto a la bitácora (mismo acceso). Corre los casos fijos de `src/application/bench/cases.ts` contra el experto conectado (usa la llave de la persona y cuesta tokens) y revisa sin experto todas las variantes de cada módulo de mueble; una variante con avisos es un error de Knotty. `npm run compare` usa los mismos casos y la misma calificación.
