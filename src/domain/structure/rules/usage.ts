...
export const doorRule: Rule = ({ design, geo }) =>
  design.piezas
    ...
    .flatMap((p): Finding[] => {
      ...
      const found: Finding[] = []
      const hinges = design.uniones.filter((u) => u.tipo === 'bisagra-cazoleta' && (u.a === p.id || u.b === p.id))
      const fitted = hinges.reduce((n, u) => n + u.herrajes.reduce((m, h) => m + (h.cantidad ?? hingesFor(height)), 0), 0)
      ...
      if (hinges.length && fitted < needed)
        found.push({
          ...
          pieces: [p.id],
          message: `${p.nombre} mide ${Math.round(height)} mm de alto y lleva ${fitted} bisagras; con esa altura van ${needed} para que no se descuelgue.`,
      ...
      if (width > ASSUMPTIONS.doors.maxWidth)
        found.push({
          ...
          pieces: [p.id],
          message: `${p.nombre} mide ${Math.round(width)} mm de ancho: una hoja tan ancha pesa en las bisagras y estorba al abrir.`,
      ...
      return [
        {
          ...
          severity: 'recomendacion',
          pieces: [p.id],
          message: `${p.nombre} cruza ${roundTo(span, 0)} mm sin nada debajo: con peso encima tiende a vencerse.`,
...
const GRAIN_SHOWS = new Set(['lateral', 'entrepano', 'piso', 'techo', 'divisor', 'puerta'])

/** R8: grain across a long piece looks odd and makes it less stiff. */
export const grainRule: Rule = ({ design, geo }) =>
  design.piezas.flatMap((p): Finding[] => {
    const box = geo.boxes.get(p.id)
    if (!box || p.veta !== 'ancho' || !GRAIN_SHOWS.has(p.rol)) return []
