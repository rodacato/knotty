...
const PERIMETER = new Set(['lateral', 'piso', 'techo'])
const RAILS = new Set(['piso', 'techo', 'entrepano', 'faja', 'zoclo'])
...
export const rackingRule: Rule = ({ design, geo }) => {
  const sides = design.piezas.filter((p) => p.rol === 'lateral').map((p) => p.id)
  if (sides.length < 2) return []
  const role = new Map(design.piezas.map((p) => [p.id, p.rol]))
  const joinedTo = (id: string, filter: (type: string, glued: boolean) => boolean) =>
    new Set(design.uniones.filter((u) => (u.a === id || u.b === id) && filter(u.tipo, u.pegamento)).map((u) => (u.a === id ? u.b : u.a)))

  // A full board in the back plane braces like a back: the spine of a bed base, screwed to both ends and the platform.
  const backs = design.piezas.filter((p) => p.rol === 'trasera' || (p.rol === 'divisor' && p.normal === 'z'))
  const rigidBack = backs.some((b) => {
    const thickness = geo.thicknesses.get(b.id) ?? 0
    const onPerimeter = (ids: Set<string>) => [...ids].filter((id) => PERIMETER.has(role.get(id) ?? 'otro')).length
    if (thickness >= 6) return onPerimeter(joinedTo(b.id, () => true)) >= 3
    return onPerimeter(joinedTo(b.id, (type, glued) => (type === 'rebaje' || type === 'canal') && glued)) >= 3
  })

  const rails = design.piezas.filter((p) => RAILS.has(p.rol) && p.apoyo === 'fijo' && sides.every((side) => joinedTo(p.id, (type) => RIGID_JOINTS.includes(type as never)).has(side)))
  const rigidFrame = rails.length >= 2 && rails.some((p) => p.rol === 'faja' || p.rol === 'zoclo')
  ...
  const height = Math.max(...sides.map((id) => geo.boxes.get(id)?.y1 ?? 0)) || design.dimensiones.alto
