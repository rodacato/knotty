...
function inferJoint(c: Contact, p: Pieza, q: Pieza, thicknesses: Map<string, number>, catalog: Catalogo): Omit<Union, 'id'> | null {
  ...
  if (back) {
    ...
    return union('', back.id, other.id, 'clavo-pegamento', [{ herrajeId: 'clavo-sin-cabeza-1', cantidad: null }])
  }
...
function hinge(door: Pieza, box: Caja, neighbours: { piece: Pieza; box: Caja }[]): Omit<Union, 'id'> | null {
  const uprights = neighbours.filter((x) => x.piece.normal === 'x' && x.piece.rol !== 'puerta')
  if (!uprights.length) return null
  const center = (k: Caja) => (k.x0 + k.x1) / 2
  const distance = (k: Caja) => Math.min(Math.abs(center(k) - box.x0), Math.abs(center(k) - box.x1))
  ...
  return union('', door.id, chosen.piece.id, 'bisagra-cazoleta', [{ herrajeId: 'bisagra-cazoleta-35-recta', cantidad: null }])
}
...
export function completeJoints(design: Diseno, catalog: Catalogo, previous?: Diseno): Diseno {
  const resolved = resolver(design, catalog)
  if (!resolved.ok) return design
  const { cajas: boxes, espesores: thicknesses } = resolved.valor
