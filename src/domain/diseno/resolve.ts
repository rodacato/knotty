...
export interface Geometry {
  ...
  measure: (cota: Cota, axis: Eje) => number
}
...
export const faceAxes = (normal: Eje) => EJES.filter((e) => e !== normal) as [Eje, Eje]

export function parseFace(face: CaraRef) {
  const [piece, which] = face.split('.')
  return { piece, axis: which[0] as Eje, side: Number(which[1]) as 0 | 1 }
}
...
export function resolveGeometry(design: Diseno, catalog: Catalogo): Result<Geometry> {
  ...
  const face = (ref: CaraRef, axis: Eje, who: string): number => {
    ...
    if (piece === 'mueble') return side === 0 ? 0 : design.dimensiones[DIMENSION_DE_EJE[axis]]
  ...
  const value = (cota: Cota, axis: Eje, who: string): number => {
  ...
  function extentOf(p: Pieza, axis: Eje): [number, number] {
    const key = `${p.id}.${axis}`
  ...
  function normalExtent(p: Pieza, axis: Eje): [number, number] {
  ...
  function faceExtent(p: Pieza, axis: Eje): [number, number] {
...
export function faceSize(box: Box, normal: Eje): [number, number] {
  const m = sizeOf(box)
