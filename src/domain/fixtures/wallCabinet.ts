...
const puerta = (lado: 'left' | 'right'): Piece =>
  makePiece({
    id: `door-${lado}`,
    name: lado === 'left' ? 'Puerta izquierda' : 'Puerta derecha',
    role: 'door',
    material: 'T18',
    normal: 'z',
    x:
      lado === 'left'
        ? extent(ref('furniture.x0', HOLGURA), partway('furniture.x0', 'furniture.x1', 0.5, -ENTRE_PUERTAS / 2))
        : extent(partway('furniture.x0', 'furniture.x1', 0.5, ENTRE_PUERTAS / 2), ref('furniture.x1', -HOLGURA)),
    y: extent(ref('furniture.y0', HOLGURA), ref('furniture.y1', -HOLGURA)),
...
export const exampleWallCabinet: Design = {
  ...
  joints: [
    ...['bottom', 'top'].flatMap((b) => ['side-left', 'side-right'].map((lat) => makeJoint(`j-${b}-${lat}`, lat, b, 'butt-screw', [{ hardwareId: 'screw-8x2', count: null }]))),
    ...['side-left', 'side-right', 'bottom', 'top'].map((b) => makeJoint(`j-back-${b}`, 'back', b, 'glue-nail', [{ hardwareId: 'brad-nail-1', count: null }])),
    ...['side-left', 'side-right'].map((lat) => makeJoint(`j-shelf-${lat}`, 'shelf', lat, 'shelf-pin', [{ hardwareId: 'shelf-pin-5', count: 2 }])),
