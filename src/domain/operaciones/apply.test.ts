...
const aplicado = (d: Diseno, ops: Operation[]) => {
  const r = applyOperations(d, ops.map((o) => Operation.parse(o)), catalogo)
...
describe('applyOperations', () => {
  it('stretching wider carries to the sides and shelves and leaves the rest alone', () => {
    const { design: diseno } = aplicado(librero, [{ op: 'cambiarDimensionGlobal', eje: 'x', valor: 900, regla: 'estirar' }])
  ...
  it('adding a divider requires splitting the shelves it crosses', () => {
    const { design: diseno } = aplicado(librero, [{ op: 'agregarPieza', pieza: divisor }])
  ...
  it('a full divider: shelves split in two with joints, and it stays valid', () => {
    ...
    const { design: diseno } = aplicado(librero, ops)
    expect(valido(diseno)).toEqual([])
  ...
  it('removing a piece freezes the cotas that referred to it and warns', () => {
    const { design: diseno, warnings: avisos } = aplicado(librero, [{ op: 'eliminarPieza', id: 'zoclo' }])
  ...
  it('moving keeps the length and a lowered shelf stays valid', () => {
    const { design: diseno } = aplicado(librero, [{ op: 'mover', id: 'entrepano-1', eje: 'y', cota: mm(300) }])
  ...
  it('changing thickness carries to what refers to it', () => {
    const { design: diseno } = aplicado(librero, [{ op: 'cambiarEspesor', ids: ['lat-izq', 'lat-der'], material: 'T15' }])
    expect(cajas(diseno).get('piso')).toMatchObject({ x0: 15, x1: 585 })
  })

  it('proportional scales the absolute cotas', () => {
    const conMm = aplicado(librero, [{ op: 'mover', id: 'entrepano-1', eje: 'y', cota: mm(400) }]).design
  ...
  it('fails without applying anything and names the operation', () => {
    const r = applyOperations(librero, [{ op: 'cambiarAnclajeMuro', valor: false }, { op: 'eliminarPieza', id: 'no-existe' }], catalogo)
  ...
  it('does not resize along the thickness axis', () => {
    const r = applyOperations(librero, [{ op: 'redimensionar', id: 'piso', eje: 'y', extremo: 'hasta', cota: mm(200) }], catalogo)
...
describe('cutList', () => {
  it('groups equal pieces', () => {
    ...
    const lista = cutList(librero, r.value)
