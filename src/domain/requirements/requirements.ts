...
export function checkRequirements(design: Design, requirements: Requirement[]): DesignError[] {
  return requirements.flatMap((r) => {
    ...
    return outside ? [error('E_REQUIREMENT', `El ${DIMENSION_LABEL[DIMENSION_OF_AXIS[r.axis]]} de ${measure} mm no respeta "${r.text}".`, { requirement: r.id, measure: measure, min: r.min, max: r.max })] : []
  })
}
