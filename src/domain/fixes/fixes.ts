...
function runnerSupportPiece(design: Diseno, catalog: Catalogo, group: string, side: 'izq' | 'der'): Operacion[] {
  ...
  const found = geo && drawerSides(design, geo.cajas).find((d) => d.group === group && d.towards === (side === 'izq' ? -1 : 1))
  ...
  const box = geo.cajas.get(found.side.id)!
  const drawer = design.piezas.filter((p) => p.grupo === group && geo.cajas.has(p.id)).map((p) => geo.cajas.get(p.id)!)
  ...
  const inColumn = [...geo.cajas.entries()].filter(([id, b]) => !drawer.includes(b) && id !== found.side.id && b.x0 < x0 + thickness && b.x1 > x0 && Math.min(b.z1, box.z1) - Math.max(b.z0, box.z0) > 0)
  ...
  const support = pieza({
    ...
    material,
    normal: 'x',
    x: desde({ tipo: 'mm', mm: Math.round(x0 * 10) / 10 }),
    y: tramo(below ? ref(`${below[0]}.y1`) : ref('mueble.y0'), above ? ref(`${above[0]}.y0`) : { tipo: 'mm', mm: Math.round(top) }),
    z: tramo({ tipo: 'mm', mm: Math.round(box.z0) }, { tipo: 'mm', mm: Math.round(box.z1) }),
    cantos: ['frente'],
  })
  ...
  return [
    ...
    { op: 'agregarUnion', union: union(`u-${group}-corredera-${side}`, found.side.id, id, 'corredera', hasHardware ? [] : [{ herrajeId: runner.id, cantidad: 1 }]) },
  ]
}
