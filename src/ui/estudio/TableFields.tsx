  ...
  return (
    <>
      <section className="flex flex-col gap-2">
        ...
        <Row label="Uso">
          <Segmented label="Uso" value={draft.use} options={USE} onChange={(use) => set({ use: use as TablePlan['use'], shelf: use === 'desk' ? false : draft.shelf, pedestal: use === 'desk' ? draft.pedestal : { side: 'none', drawers: 0 } })} />
        </Row>
      </section>
      ...
      <section className="flex flex-col gap-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <NumberField label="La cubierta sobresale" suffix="mm" value={draft.overhang} onChange={(overhang) => set({ overhang })} />
        </div>
        <Row label="Triplay">
