...
function Stroke() {
  const line = 'fill-none stroke-grafito [stroke-width:1.6] [stroke-linecap:round] [stroke-linejoin:round] animate-trazo'
  return (
    <svg viewBox="0 0 160 200" className="h-48 w-40 text-grafito" aria-hidden>
      ...
      <path pathLength={1} strokeDasharray="1" className={`${line} [animation-delay:.9s] stroke-ambar`} d="M130 20 l18 -12 v170 l-18 12" />
    </svg>
  )
}
  ...
  return (
    <main className="flex min-h-full flex-col items-center justify-center gap-8 px-6" aria-live="polite">
          ...
          return (
            <li key={e.id} className={`flex items-center gap-3 transition ${taken || inProgress ? 'text-grafito' : 'text-grafito-2/50'}`}>
              <span className={`grid size-6 place-items-center rounded-full border ${taken ? 'border-grafito bg-grafito text-hueso' : inProgress ? 'border-ambar' : 'border-linea'}`}>
                {taken ? <Check size={12} weight="bold" /> : inProgress ? <span className="size-2 animate-pulse rounded-full bg-ambar" /> : null}
              </span>
      ...
      <div className="flex flex-col items-center gap-3 text-center">
        <p className="cifras text-sm text-grafito-2">{clock(seconds)}</p>
        {delIntento >= PATIENCE && (
