...
export function ReviewGate({ stale }: { stale: boolean }) {
  ...
  return (
    <section className="flex flex-col gap-3 rounded-2xl border border-line bg-bone p-4">
      ...
      {error && (
        <p className="flex items-start gap-2 rounded-xl border border-rust/30 bg-rust/10 p-3 text-sm text-rust">
  ...
  return (
    <li className="flex items-start gap-2 py-2 text-sm">
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className={`block text-xs ${c.status === 'ok' ? 'text-graphite-2' : ''}`}>{c.detail}</span>
        {piece && (
          <button type="button" className="text-xs text-graphite-2 underline" onClick={() => select(piece)}>
            Ver en 3D
          </button>
        )}
    ...
    <section className="flex flex-col gap-3 rounded-2xl border border-line bg-bone p-4">
      ...
      {c && c.problems.length > 0 && (
        <ul className="flex flex-col gap-2">
          {c.problems.map((p, i) => (
            <li key={i} className={`flex flex-col gap-1.5 rounded-xl border bg-paper/60 p-3 ${GRAVITY[p.severity]}`}>
