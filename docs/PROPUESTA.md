...
30. El gabinete sobre patas (hueco 3 del catálogo de referencia, unos 12 productos; el primer caso es el aparador KC-APA-01, 940 × 1600 × 400 sobre cuatro patas abiertas bajo un faldón remetido). La base del gabinete suma `legs` («Con patas» en la ficha, «con patas» en los cambios) a `kick` y `floor`. **Cómo se arma** (`legBase` en `src/domain/furniture/modules/cabinet.ts`): el casco sube sobre las patas (el piso empieza a `LEG_HEIGHT` = 150 del suelo y los laterales y la trasera nacen en él; el alto de la ficha incluye las patas); en cada esquina una pata de dos capas del triplay del casco pegadas y atornilladas cara con cara, 36 × 72 ([estructura.md](carpinteria/estructura.md) §2.1, «2 × 18 = 36 × 72 mm»), remetida 30 del canto; entre ellas un faldón de 80 ([estructura.md](carpinteria/estructura.md) §2.1, «de 80–120 mm de alto») con tornillo de bolsillo a las patas, y el piso atornillado sobre patas y faldones. Si dos patas quedan a más de 1 200 mm lleva patas intermedias, una al frente y otra atrás, pegadas al faldón ([estructura.md](carpinteria/estructura.md) §7.1, «más de ≈ 1 200 mm de ancho → patas intermedias»), bajo el divisor más cercano si así ningún claro pasa del límite ([muebles-y-medidas.md](carpinteria/muebles-y-medidas.md) §1.1: «patas cerca de los divisores») y repartidas si no; y travesaños entre los faldones cada 600 como mucho (`MAX_SPAN`, el mismo de la mesa), para que el piso no cruce más de lo que aguanta una repisa ([estructura.md](carpinteria/estructura.md) §7.1: «un piso sobre patas se revisa como una repisa»). No hay pata comprable en el catálogo: todo sale del triplay, así que la lista de compra suma las piezas y las uniones traen su tornillo y pegamento. Sin referencia, con su porqué (`MODULE_SOURCES`): el alto de 150 (lo que muestra el KC-APA-01; la referencia no da alto de patas) y el remetido de 30 (el del zoclo). **Revisiones:** R1 revisa el piso sobre patas (no está en el suelo, así que no se exenta; los apoyos son laterales, patas y travesaños); R7 suma `base.legs` (recomendación, versión 2): entre dos patas a más de `ASSUMPTIONS.legs.maxSpan` (1 200), con el apoyo al centro que Knotty ya construye; una pata es una pieza en el suelo de no más de 150 × 150 (`legs.footprint`), así que vale también para lo que dibuja el experto. R4 (versión 3): sobre patas, la proporción alto ÷ fondo usa el fondo en que se apoyan las patas, no el del casco, y el «más fondo» que sugiere suma lo que se remeten. **Chat sin experto:** con tres valores, «sin X» va al único valor que dice «sin» algo: «con patas» pone patas y «sin patas» o «quítale las patas» lo deja directo al suelo; «con zoclo» y «sin zoclo» siguen igual. **Experto:** `skeleton@13` explica `legs` (y que las patas abiertas o ahusadas se arman rectas) y pide respetar la base que leyó la foto (que ya tenía `legs`; ruedas va directo, y se dice); `plan-adjust@11` nombra las tres bases. Cambió lo que lee el experto: falta correr `npm run compare`. **Lo guardado** no cambia de formato: la ficha suma un valor al enum y las sesiones viejas (`kick`, `floor`) se leen igual. **Banco sin experto:** dos variantes nuevas, «aparador con patas» (como el KC-APA-01: seis patas y dos travesaños) y «buró con patas», y todas siguen válidas y sin avisos. **El ejemplo del aparador** del inicio (paso 29) ahora va sobre patas (`base: 'legs'` en `src/domain/furniture/examples.ts`): 55 piezas en vez de 41 (sin zoclo ni apoyos del piso; seis patas de dos capas, cuatro faldones y dos travesaños), sin avisos, y la compra fija de `examples.test.ts` sigue en 3 hojas de 18 y 1 de 6, con 16 tornillos de bolsillo más; «sin patas» lo deja directo al suelo sin llamar al experto. El caso `sideboard` del banco no cambia: su petición no habla de patas. No sumé un caso «sobre patas»: el banco no puede revisar la base de la ficha que devuelve el experto, así que solo repetiría `sideboard`; conviene cuando el caso pueda esperar campos de la ficha. **Pruebas:** el aparador y los 32 modos de armar con patas, válidos y sin contactos sueltos; patas laminadas y pegadas, bolsillo en los faldones, piso a 150; pata intermedia bajo el divisor o al centro; sin travesaños el piso sobre patas se pandea (R1, claro de 716); sin patas intermedias, R7 `base.legs` con 1 468 y el apoyo al centro lo resuelve; R4 con el fondo de las patas (240 de 300). **Límites:** todas las piezas son cajas a lo largo de los ejes, así que las patas abiertas y ahusadas del KC-APA-01 salen rectas (se dice, no se finge una geometría que las revisiones no ven); el alto de las patas y del faldón no se editan en la ficha; no lleva niveladores (la referencia los recomienda; el catálogo tiene `leveling-foot`, pero no hay unión que lo cuelgue de una pieza); y R1 toma el piso como viga entre patas y travesaños, sin contar los faldones bajo sus cantos, que es del lado seguro.

31. El banco califica la estructura (primer paso de los prompts por categoría). Con el pedido del aparador (`sideboard`) el experto real dio cinco columnas y cuatro puertas, y nada lo marcó: el banco solo calificaba medidas, camino, módulo y críticos. Ahora un caso puede decir cuántas puertas, cajones y huecos abiertos pide (`parts: { doors, drawers, open }` en `src/application/bench/cases.ts`) y el resultado los compara con los del diseño (`structure`): puertas y cajones por sus piezas (`door` y `drawer-front`), así que cuenta también un diseño pieza por pieza; los huecos abiertos, por las celdas de la ficha del gabinete, porque las piezas no dicen dónde hay un hueco (pieza por pieza quedan en «?» y no se califican). Una cuenta distinta es «NO»; si ninguna difiere pero una no se pudo contar, no se sabe. El reporte de `npm run compare` suma la columna «Estructura» por caso («NO: puertas 4 (pidió 3) · cajones 3 · abiertos 3») y «Estructura como se pidió» en el resumen, solo con los casos que la piden y se pudieron contar; el banco de la app la muestra junto a las medidas, en rojo si no cuadra. Piden estructura el librero (sin puertas ni cajones), el buró, la alacena, la zapatera sin puertas, el mueble de TV y el aparador; y un caso nuevo, `sideboard-explicit`, el mismo aparador dicho columna por columna («cuatro columnas iguales; en las tres primeras, puerta abajo y arriba un cajoncito en la primera y nichos abiertos en la segunda y la tercera; en la cuarta, dos cajones abajo y un nicho arriba»): si ese sale bien y `sideboard` no, el experto leyó mal el acomodo, no las cuentas. `sideboard` se queda como está, ambiguo a propósito. No cambia lo que lee el experto, así que ningún prompt sube de versión; falta correr `npm run compare` con los dos aparadores para tener la línea base contra la que se miden los prompts por categoría. Banco sin experto: 185 de 185 limpias.

32. El tipo de mueble, con su origen, y cambiarlo (segundo paso de los prompts por categoría). El enrutamiento por tipo necesita que todo diseño tenga tipo, llegue como llegue: ejemplo, ficha, pieza por pieza o un pedido con la palabra. Antes el gabinete armado desde su ficha no tenía tipo (su ficha no dice si es librero o clóset), rearmar desde la ficha lo perdía y la foto daba texto libre. **Origen:** el diseño guarda `kindSource` junto a `kind` (`person`, `example`, `plan`, `photo`, `words`); sin tipo ni palabras en el nombre es `unknown`, que no se guarda (`kindOf` en `src/domain/furniture/kind.ts`). **Cuál gana** (`settleKind`): la persona siempre, y nada la pisa salvo ella misma; después el ejemplo o la ficha, la foto y las palabras, y a igual confianza el más nuevo. Dentro de un módulo, un uso afina la palabra del módulo sin importar quién la dijo: una ficha de gabinete y «librero» en las notas son un librero. Al diseñar (`startingKind`) la foto y las notas se leen por separado para que la foto gane a las palabras. El tipo pasa de versión en versión como el acabado (`addVersion`), así que un gabinete rearmado desde su ficha sigue siendo librero, y volver a una versión anterior regresa su tipo. **Tipos nuevos:** aparador (`sideboard`) y mueble de TV (`tvStand`), los dos del gabinete; el ejemplo del aparador dice el suyo, y «buró» ahora es buró y no cajonera (ninguna revisión los distinguía). **Elegirlo:** el selector «Tipo de mueble» va en la captura («Que Knotty lo decida» por omisión) y arriba de la pestaña Mueble, que dice de dónde salió («Lo elegiste tú.», «Lo dice el ejemplo.», «Knotty lo vio en tus fotos.»…). **Cambiarlo** (`chooseKind`): dentro del mismo módulo (aparador → mueble de TV), o en un diseño sin ficha, es una versión que conserva piezas y ficha («Ahora es un mueble de TV») y cambian las revisiones por tipo; en la mesa el uso vive en la ficha, así que elegir «Escritorio» la rearma como escritorio. De otro módulo (aparador → cama) la ficha no se convierte: se ofrece «Rehacer como cama» (`redoAs`), que diseña de nuevo con el primer pedido y las medidas de antes solo como referencia (las de un aparador no le sirven a una cama), como una versión más: lo anterior sigue en el historial, las notas de la persona se quedan y los requisitos que escribió el experto se van. Cambiar la ficha a mano o con el intérprete de modo que cambie el tipo cuenta como elección de la persona, y si el experto le cambia el uso a una mesa cuyo tipo eligió la persona, Knotty conserva el de la persona. **Experto:** el esqueleto recibe el tipo que eligió la persona («The person chose what this furniture is: un escritorio (desk).», también en el camino pieza por pieza) y `skeleton@14` le pide llenar el campo de ese tipo y ningún otro; si aun así llena otro módulo, Knotty lo diseña pieza por pieza. Es una línea más en la petición cuando la persona elige el tipo (≈ 20 tokens) y una frase en el prompt (≈ 55): falta correr `npm run compare`. El enrutamiento de los prompts por tipo es el paso siguiente. **Lo guardado** no cambia de formato: `kindSource` es opcional, los dos tipos son valores nuevos del enum y una sesión vieja se lee igual (su tipo, sin origen, cuenta como el de la ficha o el ejemplo que lo armó); comprobado abriendo en esta versión un librero guardado por la anterior. **Pruebas:** quién gana (la persona siempre, el ejemplo a la foto y la foto a las palabras, el uso afina el módulo), el tipo al diseñar con el experto simulado, el ejemplo, el cambio dentro del módulo que conserva la ficha y sobrevive a rearmarla, la mesa que se rearma, el cambio entre módulos que pide rehacer sin tocar nada, rehacer como cama y un rehacer que falla. Banco sin experto: 185 de 185 limpias.
||||||| parent of 1e8fe01 (PROPUESTA: paso 33, ajustar por ficha con solo su módulo)
33. Ajustar por ficha manda solo el módulo de la ficha (tercer paso de los prompts por categoría). Una ficha ya dice qué mueble es y el experto no puede cambiarla de módulo (`throughPlan` solo lee el campo de su módulo), pero `plan-adjust@11` describía los cuatro módulos y su esquema traía los cuatro campos. Ahora `plan-adjust@12` es una plantilla (`{{module}}`, `{{moduleChanges}}`, `{{moduleField}}`, `{{moduleRules}}`) que `planAdjustmentFor(kind)` (`src/adapters/llm/common/prompts.ts`) llena con el módulo de la ficha. La prosa escrita a mano de gabinete, cama y mesa pasa, sin cambiar una palabra, a `src/adapters/llm/prompts/modules/` (`cabinet.v1.md`, `bed.v1.md`, `table.v1.md`), cada una con sus secciones `## plan` (qué es la ficha), `## changes` (qué se puede cambiar) y `## rules` (solo el gabinete: puertas, frentes y cajones); es la carpeta `modules/` del catálogo de prompts, y el esqueleto la va a usar en el paso siguiente. Los módulos sin prosa a mano (la zapatera) salen de su esquema como antes (`moduleSummary`). El esquema de la respuesta también se enruta (`planAdjustmentFor` en `src/ports/LLMProvider.ts`): solo el campo del módulo; al leer la respuesta los demás quedan en `null`, así que la aplicación no cambia. El id dice la combinación, `plan-adjust@12+cabinet@1` (o `+shoeRack@auto` para un módulo generado, que sigue a su esquema y no tiene versión), y queda en el origen de la versión y en la bitácora exportada. **Tokens** (prompt más esquema, caracteres ÷ 3.5): antes ≈ 4 307 para cualquier ficha; ahora gabinete ≈ 2 305, cama ≈ 1 963, zapatera ≈ 1 834 y mesa ≈ 1 778, entre 2 000 y 2 530 menos por ajuste (46–59 %), sin contar el contexto, que no cambia. **Pruebas:** cada archivo de módulo se llama como su id y existe solo para los módulos escritos a mano; para cada módulo el prompt y el esquema nombran su campo y ningún otro; la prosa a mano llega entera (descripción, cambios y reglas); el esquema de cada módulo lo aceptan los modos estrictos; el adaptador pide solo el campo de la cama y lee la respuesta con los demás en `null`; y un **presupuesto de tokens** por módulo, ~5 % arriba de lo medido, que falla si crece sin querer. La prueba anti-duplicación de números ahora recorre también `prompts/modules/`. **Banco:** los pedidos de después que había («Sin zoclo», «¿Cuánto cuesta?») los resuelve Knotty sin experto, así que ningún caso llegaba al ajuste por ficha; la cama pide «Súbela a 45 cm y ponle cajones del lado izquierdo» y el aparador «Cambia el cajoncito de arriba por un nicho abierto», que el intérprete no lee y van al experto por la ficha. Cambió lo que ve el experto: falta correr `npm run compare` (al menos `bed` y `sideboard`) y compararlo con el último reporte; la columna de pedidos dice cuántas llamadas hizo cada uno.

34. El esqueleto enrutado por tipo (cuarto paso de los prompts por categoría). Diseñar costaba ≈ 5 400 tokens de prompt y esquema por llamada, y casi la mitad describía muebles que no eran el pedido. Ahora, si se sabe qué mueble es, el esqueleto le habla al experto de un solo módulo: su renglón, su guía y su campo del esquema. **De dónde sale el tipo para enrutar** (`routeKind` en la petición, desde `designFromPlan`): el que eligió la persona; si no, el de la lectura de la foto; si no, las palabras del pedido (`kindFromWords`); un «Rehacer como» manda el tipo nuevo. Un tipo sin módulo (la banca) sigue yendo directo a pieza por pieza. **Respaldo genérico:** sin tipo, `skeleton@15+all` es el esqueleto de siempre, con todos los módulos y el índice de una línea por módulo: renderizado sale idéntico, carácter por carácter, al `skeleton@14`. **Cómo se arma** (`skeletonFor` en `src/adapters/llm/common/prompts.ts`): `skeleton.v15.md` es una plantilla por secciones (`# intro`, `# all` para el genérico, `# one` para el enrutado y `# rest`, lo que se pide además de la ficha), y cada archivo de `prompts/modules/` suma `# pick` (su renglón en la lista) y `# skeleton` (su guía), movidos sin cambiar una palabra; las secciones de los módulos pasan de `##` a `#` para que la guía pueda tener sus propios encabezados, y los archivos suben a `cabinet@2`, `bed@2` y `table@2` (lo que lee el experto al ajustar una ficha no cambia, solo el id). La zapatera sigue generada desde su esquema (`modulePick`, `moduleGuide`). El esquema también se enruta (`planResponseFor`): con tipo conocido solo trae el campo de su módulo, y si el mueble no se puede armar así el experto lo deja en `null` y se diseña pieza por pieza; no hizo falta un `action: "freeform"` porque el `null` ya lo dice. El id dice la combinación (`skeleton@15+cabinet@2`, `+shoeRack@auto`, `+all`) y sale en el reporte de `npm run compare` y en la bitácora. **Partes compartidas:** no se hicieron. Revisado el texto, lo de puertas, cajones, repisas, base y anclaje vive solo en la guía del gabinete (la cama y la mesa tienen sus propios cajones, con otros campos), así que una carpeta `parts/` tendría un consumidor por parte; se sacan cuando un segundo módulo las use (patas en la mesa, idea 11). **Tokens** (prompt más esquema, caracteres ÷ 3.5): genérico ≈ 5 397 (igual que antes); gabinete ≈ 2 680, cama ≈ 2 003, zapatera ≈ 1 866 y mesa ≈ 1 748, entre 2 700 y 3 650 menos por diseño (50–68 %). **Pruebas:** el genérico lista todos los módulos y lleva toda su prosa; para cada módulo, el prompt y el esquema enrutados nombran su campo y ningún otro; la prosa a mano llega entera; presupuesto de tokens del esqueleto por módulo y genérico, ~5 % arriba de lo medido; en el caso de uso, la persona gana a la foto y la foto a las palabras, y sin nada va el genérico; y las palabras de cada caso del banco enrutan al módulo que el caso espera (el exhibidor escalonado, a ninguno). **Riesgo:** unas palabras que apunten al módulo equivocado dejan al experto sin la ficha correcta y el diseño se va pieza por pieza, que es caro; por eso el banco lo revisa caso por caso. Cambió lo que ve el experto: falta correr `npm run compare` con todos los casos y compararlo con el último reporte (válidos, razonables, estructura, segundos, tokens de salida y camino).

35. Guías por tipo de mueble, empezando por el aparador (quinto paso de los prompts por categoría). Con el esqueleto enrutado, dar más detalle a un tipo ya no le cuesta a los demás: una guía corta por uso (`src/adapters/llm/prompts/kinds/`) se suma después de la guía de su módulo cuando se sabe que el mueble es ese uso, en el esqueleto y en el ajuste por ficha, y el id lo dice (`skeleton@15+cabinet@2+sideboard@1`, `plan-adjust@12+cabinet@2+sideboard@1`). El tipo que la elige es el mismo que enruta el esqueleto (`routeKind`) y, al ajustar, el del diseño (`kindOf`, que la petición lleva en `kind`). Una guía solo entra si su uso es de ese módulo, y un uso sin guía deja el prompt como estaba. **El aparador** (`sideboard@1`) dice lo que enseñó el `KC-APA-01`, cuando el experto real dio cinco columnas y cuatro puertas con el pedido del caso `sideboard`: construir exactamente las puertas, cajones y nichos pedidos y contarlos otra vez en la ficha antes de contestar («tres puertas» son tres hojas, una por hueco, salvo que se pidan dobles); contar las columnas por lo que queda lado a lado, no por cómo se enumeró el pedido (tres puertas y una columna de cajones abajo, bajo un cajoncito y tres nichos, son cuatro columnas); la fila de arriba no tiene que alinearse con la de abajo y un nicho puede ir sobre la columna de cajones; y lo pesado abajo ([muebles-y-medidas.md](carpinteria/muebles-y-medidas.md), «Lo pesado abajo»). Sin números a mano. **Tokens:** la guía suma ≈ 255 al gabinete: esqueleto ≈ 2 935 (≈ 5 397 antes del paso 34) y ajuste ≈ 2 560 (≈ 4 307 antes del 33). **Pruebas:** cada guía es de un tipo con módulo y su archivo se llama como su id; la del aparador entra en el esqueleto después de la guía del gabinete y antes de «Everything else», y en el ajuste, con su id; un uso sin guía, el uso de otro módulo o sin módulo dejan el prompt igual; el adaptador la suma con `routeKind` y con `kind`; el aparador de ejemplo le dice su tipo al ajuste por ficha; presupuesto de tokens con la guía. El paso «¿qué le enseña este producto a la guía de su categoría?» se sumó al proceso para llevar un producto del catálogo a Knotty (fuera del repo). Cambió lo que ve el experto: falta correr `KNOTTY_CASES=sideboard,sideboard-explicit npm run compare` y comparar la estructura con el reporte de la línea base (paso 31).

36. El banco mide los tokens de entrada, por tipo de llamada y por prompt. Los pasos 33 a 35 ahorran tokens de entrada, pero el banco y `npm run compare` solo guardaban los de salida: el ahorro no se veía en ningún reporte. Ahora cada llamada del caso, la del diseño y las de los pedidos de después, queda con su tipo (esqueleto, pieza por pieza, ajuste por ficha, ajuste pieza por pieza, revisión de compra, lectura de foto), su prompt compuesto, sus segundos y sus tokens de entrada y salida (`callLog` en `BenchResult`). El reporte suma «Tokens de entrada (prom.)» al resumen y «Tokens entrada» a cada caso, y una tabla «Por tipo de llamada» con llamadas, entrada, salida y segundos promedio por tipo y por prompt (`byCallKind` en `src/application/bench/bench.ts`), donde el mismo esqueleto con y sin la guía del aparador son dos renglones; el banco de la app muestra los tokens de entrada del diseño cuando el proveedor los da (el experto simulado no los da). **Caché:** Claude cachea el prompt de sistema, y lo que lee de la caché o escribe en ella no viene en `input_tokens`; el adaptador ahora los suma (`readTokens` en `src/adapters/llm/anthropic.ts`), así que `Usage.inputTokens` es todo lo que la llamada leyó, esté o no en caché (el costo es otra cuenta). En los compatibles con OpenAI, `prompt_tokens` ya lo incluye. Lo que ve el experto no cambia. Con esto, una corrida de `npm run compare` mide los pasos 31 a 35 juntos: estructura, camino y tokens de entrada por llamada; el punto 6 del plan de prompts por categoría (que Knotty compare los conteos del pedido con la ficha y pida una corrección) espera ese reporte.

37. Lo que midió el comparativo de los prompts por categoría (`scripts/compare/results/2026-09-26-14-29-prompts-por-categoria.md`, SheLLM, dos corridas por caso, contra `2026-09-26-03-15-candidato-0-2-0.md`). **Resultado:** 26 de 26 válidos y todos por el camino esperado. En los mismos 10 casos de la base, 19.3 → 16.7 s por diseño y 1 824 → 1 286 tokens de salida (la base tiene una sola corrida por caso). **Estructura** 13 de 14: los dos aparadores, el ambiguo y el explícito, salen con 3 puertas, 3 cajones y 3 nichos en las cuatro corridas, así que el punto 6 del plan (que Knotty compare los conteos del pedido con la ficha y pida corrección) no hace falta. **Tokens de entrada reales** del esqueleto, tomando la llamada más baja de cada prompt: gabinete ≈ 4 550, aparador con su guía ≈ 4 890, cama ≈ 3 680, zapatera ≈ 3 500 y mesa ≈ 3 350, contra ≈ 13 200 del genérico (el promedio de sus dos llamadas, las del exhibidor): la proporción del ahorro de los pasos 33 a 35 se sostiene, pero las cifras absolutas que dan allí (caracteres ÷ 3.5) se quedan cortas ≈ 1.7–1.9 veces, porque el esquema JSON gasta más tokens por carácter; los presupuestos de `prompts.test.ts` siguen en esa unidad y ahora lo dicen (`approxTokens`: frenan el crecimiento, no cuentan). En seis casos una corrida leyó 4 300–7 400 tokens más con el mismo prompt, y el buró tardó 53 s contra 11: parece que SheLLM reintenta por dentro y suma el uso, sin que Knotty vea una segunda llamada; falta comprobarlo con `KNOTTY_RAW=1`. **Lo que falló y se corrige aquí:** la alacena salió con 4 puertas en 1 de 2 corridas: para poner la repisa fija a la mitad partió el hueco en dos, cada uno con sus dos hojas, y dijo «dos puertas que cubren toda la altura». La regla de construir y contar exactamente lo pedido deja la guía del aparador y pasa a la del gabinete (`cabinet@3`), con el caso: cada hueco con puerta lleva sus hojas, y una repisa a la mitad detrás de las puertas es `shelves` 1 en un solo hueco; el aparador queda en `sideboard@2` con lo suyo. La zapatera salió de 690 mm en 2 de 2 (fuera del rango del caso): tres pares de 230 sin los costados, así que por dentro no caben. No es regresión del enrutado: en la base también multiplicó pares por 230 sin costados (920 = 4 × 230) y entró en el rango por suerte. La descripción de sus medidas ahora dice que el ancho de fuera suma los costados, con el ejemplo calculado (3 pares: 726 con triplay de 18, no 690). Sigue el exhibidor con R5 (pregunta abierta). Los dos reportes (el de hoy y el de 02:49, que faltaba) entran al historial. Cambió lo que ve el experto: falta `KNOTTY_CASES=shoe-rack,wall-cabinet,sideboard KNOTTY_REPEAT=3 npm run compare`.

38. Knotty cuenta las puertas y los cajones que se pidieron (el punto 6 del plan de prompts por categoría, acotado al gabinete). Con la regla ya en `cabinet@3`, la alacena del banco siguió saliendo con 4 puertas en 1 de 3 corridas (`2026-09-26-15-01-cabinet-3-y-zapatera.md`; 2 de 5 contando la corrida anterior): el prompt solo no basta. **Qué se pidió** (`askedParts` en `src/domain/editing/intent/counts.ts`, con los números del intérprete de pedidos): se suman todas las menciones del pedido, así que «abajo tres puertas y a la derecha dos cajones; arriba un cajoncito» son 3 puertas y 3 cajones, y «sin puertas» son 0. Una parte no se revisa si alguna mención no trae una cuenta clara («con puertas», «puerta abajo», «cajones 3», «varios cajones»), si habla de hojas o puertas dobles, o de cajones «de cada lado»: una lectura equivocada mandaría una corrección que no hace falta. **Qué tiene la ficha** (`designParts`, que ahora usa también el banco): las piezas de puerta y los frentes de cajón. **La ronda:** si la ficha de un gabinete no cuadra, el esqueleto vuelve una vez con la diferencia exacta (`E_PARTS`, «Se pidieron 2 puertas y la ficha tiene 4. Cada hueco con puerta lleva sus propias hojas…»), con la etapa «corrigiendo» y el intento 1 en la bitácora; se queda la ficha más cercana a lo pedido y, si todavía difiere, el chat lo dice («Ojo: se pidieron 2 puertas y la ficha tiene 4. Si no es lo que querías, cámbialo en la pestaña Mueble.»). Solo el gabinete, porque ahí salió la evidencia y su rejilla es donde se malentiende una cuenta; la cama cuenta cajones por lado y la mesa en la cajonera. Una ficha que cuadra, o un pedido sin cuentas claras, no hace llamada extra. **Pruebas:** los pedidos del banco y los que no se deben revisar; el conteo de la ficha; la ronda que corrige, la que no alcanza y deja el aviso, y que sin diferencia no hay segunda llamada; y que en los casos del banco lo que Knotty lee nunca contradice lo que el caso espera (8 lecturas). Lo que ve el experto cambia solo en la ronda de corrección (ningún prompt sube de versión): falta `KNOTTY_CASES=wall-cabinet,sideboard,tv-stand KNOTTY_REPEAT=3 npm run compare`; la columna de intentos dice cuántas veces hizo falta la ronda. **Corrección al paso 37:** los tokens de más con SheLLM no son reintentos. Con `KNOTTY_RAW=1` (`2026-09-26-15-07-current-format.md`, el buró) cada llamada es una sola respuesta, pero su `prompt_tokens` trae `cached_tokens: 8780`, más que todo el prompt de Knotty (≈ 4 500): SheLLM envuelve al CLI de Claude, que suma su propio prompt de sistema cacheado (≈ 5 900), y según cómo lo reporte entra o no en la cuenta. Para comparar prompts con SheLLM cuenta la llamada más baja de cada prompt; con Claude u OpenAI directos no pasa. Con eso se cae también la relación que se veía entre esas corridas y el aparador sin anclar (2 de 3 en la misma corrida de las 15:01), que queda como variación: el anclaje sigue en manos del experto y R4 lo marca.

39. El aparador se ancla (`sideboard@3`). En `2026-09-26-16-26-cuenta-de-puertas.md` (que corrió en `bc2d776`, antes de la ronda de conteo del paso 38: la alacena salió con 4 puertas en 1 de 3 y 1 intento, 3 de 8 contando las corridas anteriores) el aparador salió sin anclar al muro en 2 de 3 corridas, con su estructura perfecta; con la de las 15:01 son 4 de 6 sin anclar desde `cabinet@3` y `sideboard@2`, contra 2 de 2 ancladas con `cabinet@2` y `sideboard@1` (`2026-09-26-14-29`). Ya no parece variación. Ningún cambio de esas versiones toca el anclaje; la sospecha, sin probar, es que la guía lo describe como «a low cabinet» y el experto lee «bajo» como «no se vuelca», aunque esa frase ya estaba antes. La guía suma un renglón: bajo no es seguro, a su alto normal pasa de `{{storageAnchorHeight}}` con puertas y cajones, así que `wallMounted` va en true salvo que la persona diga que no irá contra un muro. Cambió lo que ve el experto: falta `KNOTTY_CASES=sideboard,sideboard-explicit,wall-cabinet KNOTTY_REPEAT=3 npm run compare` (la alacena, para medir por fin la ronda del paso 38). Si el aparador sigue sin anclar, lo robusto es que Knotty lo ancle en el primer diseño y lo avise, como las reparaciones por reglas (la pregunta abierta sobre aplicar sola la solución).
||||||| parent of 43fae44 (PROPUESTA: paso 40, el esqueleto se reintenta una vez)
40. El esqueleto se reintenta una vez antes de rendirse. En `2026-09-26-15-01-cabinet-3-y-zapatera.md` una zapatera se fue pieza por pieza porque SheLLM falló en el esqueleto («the CLI returned no structured output», 49 s): costó 307 s, 4 intentos y 8 críticos, donde la ficha tarda ≈ 15 s. Ahora, si el proveedor falla, el esqueleto se pide otra vez igual; si contestó algo ilegible, vuelve con lo que no se pudo leer (`E_SCHEMA`), como en pieza por pieza; solo si el segundo también falla se diseña pieza por pieza. Una cancelación no se reintenta. Cada llamada lleva su número de intento en la bitácora, también la ronda de conteo del paso 38, así que en el peor caso un gabinete hace cuatro llamadas de esqueleto (≈ 1 min), todavía menos que pieza por pieza. **Pruebas:** el esqueleto que falla una vez se pide otra vez y sale por la ficha; el ilegible vuelve con su corrección; el cancelado no se repite; y el que falla dos veces se va pieza por pieza. Lo que ve el experto no cambia salvo en el reintento de una respuesta ilegible.

41. `npm run compare` enseña su avance y se compara solo contra una base. Hasta aquí era una sola prueba de muchos minutos sin salida: en un día midió código viejo dos veces, corrió sin configuración (18 fallas en 0 s, en un worktree sin `.env`) y se perdió de vista media hora. Ahora **cada caso es una prueba concurrente** (`KNOTTY_PARALLEL` a la vez, con `maxConcurrency`) que sale ✓ o × en cuanto termina, y el × dice qué no cuadró (`problemsOf`: falló, no razonable, estructura, críticos). **Antes de gastar** revisa que cada experto de `KNOTTY_MODELS` tenga su llave o dirección, y si no, se detiene sin correr nada; y **avisa qué mide** si el checkout no es `origin/main` (commits de más o de menos según el último fetch, cambios sin commit). El reporte (`.md` y `.json`) **se reescribe al terminar cada caso**, en el orden de los casos y no de llegada, así que una corrida cortada conserva lo hecho. **Base:** `scripts/compare/baseline.json` es la única corrida en git; `KNOTTY_SAVE_BASELINE=1` fija la corrida como base y el reporte trae **Contra la base**, caso por caso y del mismo modelo: corridas, viables, razonables, estructura, segundos y tokens de salida en promedio, y la entrada más baja («base → ahora», una cifra si no cambió; «sin base» si el caso es nuevo). `KNOTTY_BASELINE` apunta a otra corrida o a `none`. La tabla por tipo de llamada suma la entrada **mínima**, la que cuenta con SheLLM. Lo que arma el reporte vive en `src/application/bench/report.ts`, con pruebas; el script solo corre. **Los reportes dejan de subirse** (`scripts/compare/results/` en `.gitignore`); los que ya estaban quedan como historial. Resultados que cierran pasos anteriores, de las corridas de hoy con SheLLM: la ronda de conteo del paso 38 dejó la alacena con 2 puertas en 3 de 3 (una necesitó la corrección, ≈ 6 s más), y con `sideboard@3` el aparador salió anclado, viable y con su estructura en 6 de 6 (antes 4 de 6 sin anclar). La primera base se fija con una corrida completa con el `compare` nuevo.

43. R5 juzga cada caja (el escuadrado del exhibidor escalonado, la pregunta abierta). El exhibidor de la base (dos corridas, las dos con R5 crítico) es, como lo dibuja el experto, tres banquitos de 250, 500 y 750 mm: cada escalón una repisa atornillada a tope sobre sus dos costados, sin trasera ni faldón. Se descuadra de verdad, pero la regla estaba mal planteada: exigía un travesaño unido a los seis costados a la vez (imposible en un escalonado, aunque cada escalón tuviera su faldón), medía el alto del mueble entero (los escalones de 250 y 500 quedan bajo el umbral de 600 de [estructura.md](carpinteria/estructura.md) §2) y su solución pedía un techo que no hay. **Cajas** (`baysOf` en `src/domain/checks/structure/rules/racking.ts`): si una pieza une todos los costados (el techo de un gabinete, la plataforma de una cama) el mueble es una sola caja y se juzga igual que antes, con la misma trasera y los mismos travesaños; si ninguna los une, cada dos costados paralelos unidos por una misma pieza son una caja, con su alto, sus travesaños rígidos y su trasera: cualquier tablero vertical atravesado a sus costados y unido a uno (una trasera, un espinazo, el costado de otro escalón). Un aviso por caja, con sus dos costados. **La solución que Knotty construye** para una caja de un mueble con varias: fajas bajo la pieza más alta que une sus dos costados, con tornillo de bolsillo; atrás, y también adelante si la caja no tiene ningún travesaño rígido (como el marco de una mesa) y no hay puertas ni cajones que estorben. Un mueble de una sola caja sigue con la faja trasera bajo su techo. **Con el exhibidor real** (`steppedStand.fixture.json`): tres cajas de 732, 482 y 232 mm, solo la alta crítica; las fajas la escuadran y no tocan las otras. R5 pasa a la versión 2: lo aceptado con la regla vieja se vuelve a preguntar una vez. **Banco sin experto** y ejemplos: sin cambios (todos son de una sola caja). Lo que ve el experto no cambia. Queda de la pregunta abierta: aplicar sola la solución en el primer diseño, como las reparaciones por reglas, para que el exhibidor salga viable sin que la persona toque nada; y que la regla cuente los `brace` diagonales que ponga el experto.
...
- R5 en un mueble abierto que no es caja: desde el paso 43 se juzga por caja y tiene solución construible (fajas). Queda: ¿aplicarla sola en el primer diseño, como una reparación por reglas? Y la regla todavía ignora los `brace` diagonales que ponga el experto.
