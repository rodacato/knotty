...
42. La primera base y el resumen en la terminal. `scripts/compare/baseline.json` (#105) es una corrida completa con SheLLM en `8475ce9`, dos por caso: 26 de 26 válidos y razonables, estructura 14 de 14 y 24 de 26 viables (las dos que no, el exhibidor con R5, la pregunta abierta; su peor corrida, pieza por pieza, leyó ≈ 131 000 tokens en 284 s). Esqueleto por módulo, la entrada más baja: gabinete ≈ 4 670, cama ≈ 3 630, zapatera ≈ 3 580, mesa ≈ 3 350, aparador con su guía ≈ 5 090 y el genérico ≈ 8 580. Con la base, el `compare` imprime la línea de cada caso en cuanto termina, directo a stdout porque la consola de Vitest atribuye mal una línea entre casos concurrentes, y al final el resumen y **Contra la base** (`terminalSummary`), sin abrir el reporte. El exhibidor se queda en × mientras R5 no se resuelva: es lo honesto.
